...
router.get('/add-product', function(req, res, next) {
    ...
    Category.find(function(err,categories){
        res.render('admin/add_product', {title,desc,price,categories});
    });
});
...
const isValidator = [
    ...
    check('image', 'Image must have a value.').custom(isImage)
];
...
router.post('/add-product', isValidator,
    function(req, res, next) {
        ...
        if(errors.length != 0) {
            Category.find(function(err,categories) {
                res.render('admin/add_product', {errors,title,desc,price,categories});
            });
        }
        else {
...
router.get('/edit-product/:id', function (req, res, next) {
    ...
    Category.find(function(err,categories){
        Product.findById({_id: req.params.id}, function (err,p) {
            if(err){
                console.log(err);
                res.redirect('/admin/products');
            }else {
                ...
                fs.readdir(galleryDir, function(err,files){
                    if(err){
                        ...
                        res.render('admin/edit_product', {
                            ...
                            image: p.image, galleryImages : galleryImages, id: p._id
                        })
                    }
                })
            }
